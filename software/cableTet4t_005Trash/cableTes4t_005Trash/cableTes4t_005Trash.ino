...
void checkMachineState(){
  ...
  if(checkingCableStatus ==true & isMoving ==false && minLimit!=1){
    ...
    if( millis()-tmpDelayVacuum <5000){
      char *msg[] ={"","Vacuum initialized ..","Please wait",""};
      printMessage(msg);  
      return;
    }
...
boolean checkCableStillPresent(){
  int s1 = analogRead(c1);
  float current1 = s1 * (5.0 / 1023.0 );
...
void startCheckingCable(){
  ...
  float current1 = (5.0 - s2 * (5.0 / 1023.0 ))/0.670;
 Serial.println(current1);
